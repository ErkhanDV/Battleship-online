...
const AIStateSlice = createSlice({
  ...
  reducers: {
    ...
    setTurnToDestroy(state, action: PayloadAction<number>) {
      state.turnToDestroy = action.payload;
    },

    setHitted(state, action: PayloadAction<number>) {
      state.hitted = action.payload;
    },
  },
});
